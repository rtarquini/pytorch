...
bool AliasDb::moveAfterTopologicallyValid(Node* movePoint, Node* n) {
  return tryMove(movePoint, n, MoveSide::AFTER, /*dryRun=*/false);
}

bool AliasDb::couldMoveAfterTopologically(Node* movePoint, Node* n) {
  return tryMove(movePoint, n, MoveSide::AFTER, /*dryRun=*/true);
}

bool AliasDb::moveBeforeTopologicallyValid(Node* movePoint, Node* n) {
  ...
  return tryMove(movePoint, n, MoveSide::BEFORE, /*dryRun=*/false);
}

bool AliasDb::couldMoveBeforeTopologically(Node* movePoint, Node* n) {
  return tryMove(movePoint, n, MoveSide::BEFORE, /*dryRun=*/true);
}
...
bool AliasDb::tryMove(
  ...
  if (splitToMoveAndDeps) {
    // Move `toMove`
    move(movePoint, toMove, moveSide);
    ...
    for (auto n : workingSet.nodes()) {
      move(curNode, n, reversed);
      curNode = n;
    }
  } else {
    // Just append/prepend everything to `movePoint`
    for (auto n : workingSet.nodes()) {
      move(curNode, n, moveSide);
      curNode = n;
    }
  }
  return true;
...
void AliasDb::move(Node* movePoint, Node* toMove, MoveSide moveSide) {
  switch (moveSide) {
